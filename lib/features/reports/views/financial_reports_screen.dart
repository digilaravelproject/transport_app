...
class FinancialReportsScreen extends GetView<ReportsController> {
  const FinancialReportsScreen({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    final reports = controller.recentReports.where((r) => r.type == 'Financial').toList();
    ...
    return AppScaffold(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          children: [
             if (reports.isEmpty)
                const Center(child: Padding(padding: EdgeInsets.all(32), child: AppText('No financial reports found.')))
             else
                ListView.builder(
                  shrinkWrap: true,
